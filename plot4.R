##Reading and creating the energy consumption data subset

power <- read.table("household_power_consumption.txt",skip=1,sep=";")
names(power) <- c("Date","Time","Global_active_power","Global_reactive_power","Voltage","Global_intensity","Sub_metering_1","Sub_metering_2","Sub_metering_3")
subpower <- subset(power,power$Date=="1/2/2007" | power$Date =="2/2/2007")

##To transform the Date and Time vars from characters into objects of type Date and POSIXlt 
...
##to make the composite plot with lots of graphics

par(mfrow=c(2,2))

##creates the basic plot function that calls different plot functions to build the 4 plots that form the graph
